...
为降低一次性生成与加载的成本，可在 Schema/缓存内支持多个细节档位：

- **层级定义**
  - `LOD0`（极简）：仅基础形状 + 主色块，无磨损/贴花；供远景、缩略图或性能紧张模式。
  - `LOD1`（标准）：包含材质纹理、接缝、简化的光照与少量磨损；作为默认档。
  - `LOD2`（精细）：完整流水线输出，包含所有磨损、贴花、动态遮罩、微光效等。
- **生成策略**
  - 素材生成库根据请求的 `LOD` 跳过或降级某些步骤（例如 `LOD0` 不执行步骤 3/5/6，`LOD1` 删减贴花/磨损数量）。
  - `schemaHash` 按 `(schemaParams, lod)` 计算，避免不同档位混淆。
- **缓存布局**
  - `AssetEntry` 新增 `uint8_t lodLevel` 字段；同一对象可在同一 `.ppab` 中存多档位，或独立文件按需加载。
  - TOC 允许 `keyHash + lod` 组合检索；若预烘焙里缺某档位，渲染库可回退到低一档或走即时生成。
- **渲染使用**
  - 渲染库依据相机缩放、对象屏幕尺寸、性能预算选择适当 LOD（例如远处门采用 `LOD0`，近景切换到 `LOD2`，在模式切换时可渐进更新）。
  - 在 Sandbox 调试模式提供覆盖选项，方便验证不同档位输出差异。
- **生成/加载优化**
