# 渲染模式与共享内核（Sandbox/Game 共用）

目标：在不改变 Runtime 的前提下，复用一套 2D 地图渲染内核（彩色矩形/Portal/图标/对话泡），以“模式策略 + 叠加层插件 + 数据过滤”区分 Sandbox 与 Game 的呈现与信息边界。
...
- Game 专用：知识雾（未见弱化/隐藏）、论证包一致性红点（仅冲突提示）。
