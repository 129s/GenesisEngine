...
| 社交动作（SocializeWithAgent） | 同上（链路） | `include/genesis/agents/ActionSystem.hpp`、`src/agents/ActionSystem.cpp` | Spec | 社交作为一种行动队列任务执行：接近 partner → 定时社交 → partner 接受时双方降低 Social need（否则失败）→ 写入社交 outcome。 |
| 社交协商（Meet v0：propose/accept/timeout） | `docs/architecture/proposals/agents/agent-model-sascol.md` | `include/genesis/agents/Meetings.hpp`、`src/agents/NeedSatisfier.cpp`、`src/simulation/SimulationContext.cpp` | Spec | 以 inbox + meet state 实现最小会合承诺；接受后双方赶赴固定交互点并等待；到期未会合写入失败 outcome，用于 partner reliability 学习。 |
| 社交信念传播（通过社交事件共享 beliefs） | 同上（链路） | `src/agents/LearningSystem.cpp`、`include/genesis/agents/Outcomes.hpp` | Spec | 社交成功时写入 `SocialInteractionOutcome{success=true}`，LearningSystem 在同 tick 内对社交 pair 做缺货风险 beliefs 的对称混合（避免顺序依赖）。失败（Reject/Timeout/NoShow）不会触发信念共享。 |
...
| Commitment v0（短期目标承诺） | `docs/architecture/proposals/agents/agent-model-sascol.md` | `include/genesis/agents/Commitments.hpp`、`src/agents/NeedSatisfier.cpp`、`src/simulation/SimulationContext.cpp` | Spec | 用显式承诺替代“keep previous target”的特殊逻辑；在 hold 窗口内切换目标需要更大 margin，窗口外弱化。 |
...
| SASCOL（State→Affordance→Selection→Commitment→Outcome→Learning） | `docs/architecture/proposals/agents/agent-model-sascol.md` | 见各子项 | Proposal | 已开始落地 Phase 0/2：Affordance 与 Commitment v0 已有最小实现；Negotiation/合同等仍未落地。 |
