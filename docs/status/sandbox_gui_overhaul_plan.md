...
2. **视觉主题与设计令牌落地**  
   ...
   - ✅ 2025-10-22：骨架线框统一为 2px，并通过控制栏/状态栏单侧描边避免多重叠加；像素对齐的描边保持与 Dock 分割线一致的锐利度，同时关闭默认抗锯齿边框；状态栏描边改由 viewport 前景层绘制以消除 1px 缝隙；界面外围新增 2px 外框以强化整体块面感，状态栏与主体内容之间不再留缝。
3. **Scene View 与附加层整合**  
