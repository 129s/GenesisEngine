...
2. **视觉主题与设计令牌落地**  
   ...
   - ✅ 2025-10-22：骨架线框统一为 2px，并通过控制栏/状态栏单侧描边避免多重叠加；像素对齐的描边保持与 Dock 分割线一致的锐利度，同时关闭默认抗锯齿边框；控制栏/状态栏高度统一按整数像素对齐且状态栏描边改由 viewport 前景层绘制以消除 1px 缝隙；界面外围新增 2px 外框以强化整体块面感，状态栏与主体内容之间不再留缝。
3. **Scene View 与附加层整合**  
...
4. **Browser 框架重建**  
   ...
   - ✅ 2025-10-22：Browser 树条目取消垂直间距并启用行内填充，根节点 `data/` 不再显示；详情区改为独立卡片并固定在底部，目录树滚动互不影响，同时为叶子节点统一应用背景底色，提升可读性。  
   - ✅ 2025-10-22：修复 Browser 树样式变量重复定义导致的编译错误，保持布局调整后的构建可用。  
   - ✅ 2025-10-23：Browser 详情卡片沿用目录背景并补充上下护栏、横向边距与自动换行，解决标题及长文本被裁切问题；目录树支持单击展开/折叠并针对不同文件类型应用差异化叶子节点底色，同时移除顶部根路径提示。  
