# GenesisEngine · 架构文档索引

> 面向内部开发者，聚焦运行时契约、世界模型与 Agent 行为设计。愿景与原则见 [meta/vision.md](meta/vision.md)，统一路线图见 `../roadmap/README.md`。

## 阅读导航
- **Meta（原则与方法论）**
  - [meta/vision.md](meta/vision.md)：长期愿景、设计原则与成功标准。
  - [meta/emergence-and-regression.md](meta/emergence-and-regression.md)：涌现叙事如何与回归/指标体系共存。
  - [meta/motif-governance.md](meta/motif-governance.md)：母题库治理与正交性量化检查。
- **Foundation**
  - [foundation/architecture-overview.md](foundation/architecture-overview.md)：分层结构、运行循环、并发/数据流边界。
  - [foundation/runtime-api.md](foundation/runtime-api.md)：运行时控制/查询协议、Telemetry/WorldAtlas 契约。
  - [foundation/telemetry-schema.md](foundation/telemetry-schema.md)：TickTelemetry 字段字典与 schema 演进记录（GUI/工具链消费基线）。
  - [foundation/worldline-chronicle.md](foundation/worldline-chronicle.md)：世界线编年史（宏观证据输出与对比）。
- **World Layer**
  - [world/world-model.md](world/world-model.md)：Map 图（有向）+ Scene 分组 + Interaction/Portal 与坐标契约。
  - [world/world-generation.md](world/world-generation.md)：生成流程（maps/map_edges 与每图 scenes/interactions/portals）、校验与产出。
  - [world/world-representation.md](world/world-representation.md)：渲染表示（Atlas/Tilemap 仅表现）、与运行时解耦的消费准则。
  - [world/resource-economy-workshops.md](world/resource-economy-workshops.md)：资源经济 v2（工坊配方、生产作业时间/并行）的数据契约与语义。
  - [world/chunked-tile-graph.md](world/chunked-tile-graph.md)：大地图/分块加载探索笔记。
- **Agents / Behavior Layer**
  - [agents/agent-model-v0.md](agents/agent-model-v0.md)：Agent 模式 v0（已落地规范，代码对齐）。
  - [agents/agent-implementation-status.md](agents/agent-implementation-status.md)：Agent 文档-代码对齐矩阵（做了什么/没做什么）。
  - Proposal：[`proposals/agents/agent-personality-big5.md`](proposals/agents/agent-personality-big5.md)（Big5/Traits/数据驱动 Needs，未落地）。
- **Interface & Rendering**
  - Sandbox GUI：[`interface/sandbox/sandbox-gui.md`](interface/sandbox/sandbox-gui.md)、[`sandbox-gui-sim-loop.md`](interface/sandbox/sandbox-gui-sim-loop.md)、[`sandbox-gui-scene-unification.md`](interface/sandbox/sandbox-gui-scene-unification.md)、[`inspector-panel.md`](interface/sandbox/inspector-panel.md)。
  - 渲染体系：[`interface/rendering/rendering-modes.md`](interface/rendering/rendering-modes.md)、[`interface/rendering/zero-asset-rendering.md`](interface/rendering/zero-asset-rendering.md)；更多细节见 `../rendering/README.md`。
  - CLI（历史参考）：[`archive/graph-to-grid.md`](archive/graph-to-grid.md)。
- **Proposals**
  - [`proposals/README.md`](proposals/README.md)：未落地提案索引（与 Spec 分离）。

## 维护约定
- 新协议/数据结构发布时，请同步更新对应文档并在提交信息中引用。
- 保持术语一致：Scene/Interactive、Global/Local Tile Coordinates、Need/Motive/Action 等关键概念应与代码实现对齐。
- 交互式前端（GUI/游戏）与 Headless 工具链共享同一 Runtime 契约：回归/批跑以 `genesis-runtime-cli` 为主，GUI 作为可选调试前端按需启用。
- 文档默认使用中文描述，第一次出现的英文名词需给出解释或中英文并列。
